...
describe('Starting tests for my-angular', function() {
  ...
  it('website title should be my-angular', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('my-angular');
    })
  });

  it('network-name should be my-block-chain@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('my-block-chain@0.0.1.bna');
    });
  });

  it('navbar-brand should be my-angular',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('my-angular');
    });
  });
